...
app.post('/book', (req, res) => {
    try{
        books[`${lastId+1}`] = { id: lastId+1, title: req.body.title, author: req.body.author, description: req.body.description };
        lastId++;
...
app.put('/book/:id', (req, res) => {
    try{
        if(books[`${req.params.id}`]){
            books[`${req.params.id}`] = { id: `${req.params.id}`, title: req.body.title, author: req.body.author, description: req.body.description };
            return res.status(200).send({id: lastId});
        }
        else{
            return res.sendStatus(400);
