...
describe('EditBook container', () => {
  let editBooksContainer: ReactTestInstance
  ...
  beforeAll(async () => {
    ...
    editBooksContainer = await waitFor(async () => getByTestId('editBook'))
  })
  ...
  test('Load the navbar', () => {
    const navbar = editBooksContainer.findByType(Navbar)
    expect(navbar).toBeTruthy()
  })

  test('Load the form', () => {
    form = editBooksContainer.findByType(Form)
  ...
  test('The form info is correct', () => {
    ...
    expect(inputs[0].findByType(TextInput)).toHaveProp('defaultValue', 'Title')
    expect(inputs[1].findByType(Text)).toHaveTextContent('Autor')
    expect(inputs[1].findByType(TextInput)).toHaveProp('defaultValue', 'Author')
    expect(inputs[2].findByType(Text)).toHaveTextContent('Descripción')
    expect(inputs[2].findByType(TextInput)).toHaveProp(
      'defaultValue',
      'Description'
    )
  })
